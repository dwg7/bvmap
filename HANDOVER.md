...
9. **完了(2026-09-20、[0036](docs/decisions/0036-sprite-grayscale-raster-exception.md))**: スプライト(GSIアイコン119個)のグレースケール化。`generator/cool_transform.py`(0030の式を初めて実コード化)+`generator/sprite_grayscale.py`で`sprite/std.png`→`sprite/bvmap-starlight.png`(+`@2x`)を生成、ブラウザでの実描画確認済み。`hfu/stars`の`assets/sprites.json`方針(SVGソース+Martin焼成、PNGミラー禁止)と食い違うため「ラスタ例外」として提案する前提(藤村さんの判断)。**残作業**: `hfu/stars`へのPR、ホスト後の`style/bvmap-starlight.json`の`sprite`フィールド切り替え、GSIアイコンのライセンス確認(0014から未解決のまま持ち越し)
