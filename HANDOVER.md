...
7. `hfu/stars`へのPR作成([0006](docs/decisions/0006-hfu-stars-is-already-master-repo.md))——**完了(2026-09-20)**: [hfu/stars#12](https://github.com/hfu/stars/pull/12)(`styles/bvmap-starlight.json`単体、CONTRIBUTING.mdの手順通りコントリビューター側で提出)がマージされ、本番配置済み。`https://stars.optgeo.org/style/bvmap-starlight`で配信中(123層、`stars.optgeo.org/catalog`でスタイル8件・スプライト3件に更新済みを確認)。`sprite`/`glyphs`ともstars自前配信([0036](docs/decisions/0036-sprite-grayscale-raster-exception.md)〜[0038](docs/decisions/0038-sprite-modification-attribution.md))。副産物: 配信中スプライトのアイコン単位crop比較で`stars`側の検証コードのバグ(Pillow 10+の`getbbox()`既定値`alpha_only=True`)を発見、`stars`が修正・`assets/sprites.json`訂正済み(ADR 0036に記録)。**次の課題(stars側からの申し送り)**: 今回のPRは`hfu`アカウントから提出したため自己承認扱いになりGitHubの仕様上レビュー承認が付けられなかった(実害はないが、今後`dwg7`側アカウントから出せると承認の記録が残ってきれいとのこと)
   - **今後の反映は一度きりでなく継続的**: 配色磨き上げ(⑥)・「見えないところの改善」を進めるたびに、都度`hfu/stars`へPRする運用になる
...
11. **完了(2026-09-21、[0039](docs/decisions/0039-group-c-tokens.md))**: [0032](docs/decisions/0032-literal-layers-generator-migration-plan.md)訂正後の推奨順序4(Group C残り6層)を実装。新規トークン`gray_99`/`gray_35`/`gray_27`、`literal_count`が15→9に減少。**効果の実測**: 県境線をbvmap-dark/starlightで並べてブラウザ確認したところ、見た目の変化はほぼ知覚できない(`cool_transform`が暗い値を意図的に控えめ(3%)にしか明るくしない設計のため)。この種の層を「見た目の磨き上げ」目的で追いかける効果は薄く、価値があるとすれば構造的な一貫性(生のGSI値が残る箇所を減らす)側——次に着手する際はこの実測を踏まえて優先度を判断すること。**残り**: Group A(5層、`patches`→`standalone`移動、注釈の置き場所が未解決)、Group E(2層、`step_outputs`の`{from:}`参照拡張)
