...
7. `hfu/stars`へのPR作成([0006](docs/decisions/0006-hfu-stars-is-already-master-repo.md))——**PR提出済み(2026-09-20)**: [hfu/stars#12](https://github.com/hfu/stars/pull/12)で`styles/bvmap-starlight.json`単体を提出(CONTRIBUTING.mdの手順通り、コントリビューター側で提出——ゲートキーパーセッションに代行を頼まない作法)。`stars`が実際にsprite(`https://stars.optgeo.org/sprite/bvmap-starlight`)・glyphs(`https://stars.optgeo.org/font/{fontstack}/{range}`)の自前配信を用意し、こちらは`starlight-input.yaml`の`hosting:`セクション経由でstyle側を配線([0036](docs/decisions/0036-sprite-grayscale-raster-exception.md)〜[0038](docs/decisions/0038-sprite-modification-attribution.md))。配信中のスプライトを手元のPNGとアイコン単位で突き合わせ、アルファは119/119完全一致・RGBはエッジ付近で最大±10〜30程度の微小なずれ(resvg再構築時のリサンプリングらしき挙動)を発見、`stars`へ報告済み(ブロッキングではないと判断)。**残作業**: `stars`側のPRレビュー・マージ・本番反映(藤村さんの承認後にMartin再起動)を待つ
