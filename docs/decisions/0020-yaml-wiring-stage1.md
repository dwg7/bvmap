...
## コードレビューでの修正(2026-09-20)

`generator/`配下の全ファイルに対してコードレビューを実施し、10件のCONFIRMED/PLAUSIBLE指摘を得た。うち「早すぎる最適化/防御ではないか」を検討した結果、**直近の作業に直結する3件のみ**を修正した(残りは今回は見送り——起こり得ないシナリオへの防御や、この規模のワンショットスクリプトには過剰な防御だったため):

1. **`load_input.build_patches()`の`color_overrides`ガードを、キーの有無だけでなく値の形まで見るように修正**。従来は`prop not in paint`だけを見ており、`paint[prop]`が`step`/`case`等の式(リスト)だった場合、将来誰かが`background`のような層に色の上書きを追加すると**黙って式全体を1つの色に置き換えてしまう**リスクがあった。`isinstance(paint[prop], list)`なら`ValueError`で明確に止まるように変更(実際にbackgroundへの上書きを試すテストで確認済み)
